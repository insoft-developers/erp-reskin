    ...
    <script>
        ...
        $(document).ready(function() {
            ...
            $(".rb-input").click(function() {
                ...
                if (nilai == 1) {
                    ...
                    $("#bagian_total").html(pp);

                    var lk = '';
                    lk += '<div class="col-md-4"><strong>Nama Bahan Baku</strong></div>';
        ...
        function tambah_item() {
            $.ajax({
                ...
                success: function(data) {
                    ...
                    for (var i = 0; i < data.data.length; i++) {
                        html += '<option value="' + data.data[i].id + '">' + data.data[i].material_name + '-' +
                            data
                            .data[i].unit + '</option>';
                    }
                    html += '</select>';
        ...
        $(document).ready(function() {
            ...
            $("#modal-tambah form").submit(function(e) {
                ...
                $("#loading-image").show();
                var id = $("#id").val();
                if (save_method == 'add') url = "{{ url('material_purchase') }}";
                else url = "{{ url('material_purchase') }}" + "/" + id;
                $.ajax({
                    url: url,
                    type: "POST",
            ...
            $("#form-upload-material-purchase").submit(function(e) {
                e.preventDefault();
        ...
        function add_data() {
            ...
            $(".modal-title").text("Tambah Transaksi Bahan Baku");
            resetData();
        }
        ...
        var table = $('#table-material-purchase').DataTable({
            ...
            ajax: "{{ route('material.purchase.table') }}",
            order: [
                [1, "desc"]
            ],
            columns: [{
        ...
        function editData(id) {
            ...
            $.ajax({
                url: "{{ url('material_purchase') }}" + "/" + id + "/edit",
                type: "GET",
                dataType: "JSON",
                success: function(data) {
                    console.log(data);
        ...
        function sync(id) {
            var csrf_token = $('meta[name="csrf-token"]').attr('content');
            $.ajax({
                url: "{{ route('material.purchase.sync') }}",
                type: "POST",
                dataType: "JSON",
                data: {
                    "id": id,
        ...
        function upload_material_purchase() {
