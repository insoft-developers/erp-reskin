@if ($view == 'product-edit')
    <script>
        var varian_group_ready = "{{ $jumlah_group }}";
        let n = +1 + +varian_group_ready;
        let i = 0;
        var varian_item_ready = "{{ $jumlah_varian }}";
        let varian_item_index = +1 + +varian_item_ready;

        var com_index_ready = "{{ $jumlah_komposisi }}";

        com_index = com_index_ready;
        ...
        function add_varian_group() {
            ...
            html +=
                '<td width="15%"><input value="0" type="number" id="max_quantity_' + varian_item_index +
        ...
        $("#is_variant").change(function() {
            ...
            if (varian == 2) {
                ...
                html +=
                    '<td width="15%"><input value="0" type="number" id="max_quantity_1" name="max_quantity[]" placeholder="max qty" class="form-control cust-control"></td>';
                html +=
        ...
        $("#is_manufactured").change(function() {
            ...
            if (nilai == 1) {
                ...
                $('#buffered_stock').html(
                    '<option value="0"> No - Jangan Gunakan Stok</option><option value="1">Yes - Gunakan Stok</option>'
                );
            } else {
                $("#manual_hpp").hide();
                $("#createdBy").show();
                ...
                $.ajax({
                    url: "/get_composition_product/" + product_id,
                    type: "GET",
                    success: function(data) {
                        elemoption += data;
                        $("#composition-container").html(elemoption);
