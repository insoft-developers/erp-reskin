<form method="POST" id="formEdit" action="{{ route('crm.customer.update', $data->id) }}" enctype="multipart/form-data">
    @csrf
    @method('PUT')
        ...
        <div class="modal-body" id="editCustomer">
            ...
            <div class="mb-4">
                <div class="col-12 col-xxl-12">
                    ...
                    <input type="email" class="form-control @error('email') is-invalid @enderror"
                        id="email" name="email" value="{{ old('email', $data->email) }}" placeholder="">
                    </input>
                    @error('email')
                        <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
            ...
            <div class="mb-4">
                <div class="col-12 col-xxl-12">
                    ...
                    <input type="text" class="form-control @error('phone') is-invalid @enderror"
                        id="phone" name="phone" value="{{ old('phone', $data->phone) }}" placeholder="">
                    </input>
                    @error('phone')
                        <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
            ...
            <div class="col-span-4 mb-3">
                ...
                <select id="customer-province" name="province_id" class="form-control">
                    <option value="{{ $data->province_id }}" selected>{{ $data->province->province_name ?? null }}</option>
                </select>
            </div>
            <div class="col-span-4 mb-3">
                <label for="customer-city" class="col-form-label">Kota: </label>
                <select id="customer-city" name="city_id" class="form-control">
                    <option value="{{ $data->city_id }}" selected>{{ $data->city->city_name ?? null }}</option>
                </select>
            </div>
            <div class="col-span-4 mb-3">
                ...
                <select id="customer-district" name="district_id" class="form-control">
                    <option value="{{ $data->district_id }}" selected>{{ $data->district->subdistrict_name ?? null }}</option>
                </select>
            </div>
            <div class="mb-4">
                <div class="col-12 col-xxl-12">
                    <label class="form-label" for="kelurahan">Kelurahan <span
                            class="text-danger">*</span></label>
                    <input type="text" class="form-control @error('kelurahan') is-invalid @enderror"
                        id="kelurahan" name="kelurahan" value="{{ old('kelurahan', $data->kelurahan) }}" placeholder="">
                    </input>
                    @error('kelurahan')
                        <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
            ...
            <div class="mb-4">
                ...
                <textarea name="alamat" id="alamat" class="form-control" cols="30" rows="10">{{ old('alamat', $data->alamat) }}</textarea>
            </div>
        </div>
