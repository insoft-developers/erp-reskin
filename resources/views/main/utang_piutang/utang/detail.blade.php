            ...
            <tbody>
                @foreach ($data->debt_payment_history as $item)
                    <tr>
                        <td>
                            @if ($item->sync_status == 1)
                                <a href="javascript:void(0)" class="delete btn btn-info btn-sm m-1"
                                    onclick="payment_unsync({{ $item->id }})"><i
                                        class="fa fa-scissors"></i>&nbsp;Unsync</a>
                            @else
                        ...
                        @elseif($item->account_code_id == 10)
                            <td>{{ $item->payment_cost->name ?? null }}</td>
                        @elseif($item->account_code_id == 11)
                            <td>{{ $item->payment_income->name ?? null }}</td>
                        @elseif($item->account_code_id == 2)
                            <td>{{ $item->payment_fixed->name ?? null }}</td>
                        @elseif($item->account_code_id == 8)
                            <td>{{ $item->payment_cogs->name ?? null }}</td>
                        @elseif($item->account_code_id == 9)
                            <td>{{ $item->payment_selling->name ?? null }}</td>
                        @else
                            <td>{{ $item->payment_from->name ?? null }}</td>
                        @endif
