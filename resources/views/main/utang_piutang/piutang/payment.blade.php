<form method="POST" id="formPayment" action="{{ route('piutang.payment', [$data->id]) }}" enctype="multipart/form-data">
    @csrf
        ...
        <div class="modal-body">
            <div class="mb-4">
                <div class="col-12 col-xxl-12">
                    <label class="form-label" for="note">Tanggal </label>
            ...
            <div class="mb-4">
                <div class="col-12 col-xxl-12">
                    <label class="form-label" for="payment_to_id">Bayar Ke<span class="text-danger">*</span></label>
                    <select type="text" class="form-control @error('payment_to_id') is-invalid @enderror"
                        id="payment_to_id" name="payment_to_id">
                        <option value="">Pilih</option>
                        @foreach ($dt as $d)
                            <option value="{{ $d['id'] }}_{{ $d['account_code_id'] }}">{{ $d['name'] }}</option>
                        @endforeach
                    </select>
                    @error('payment_to_id')
                        <div class="invalid-feedback">{{ $message }}</div>
...
<script>
    $("document").ready(function() {
        $("#payment_to_id").select2({
            dropdownParent: $("#modal-bayar-piutang")
        });
    });
