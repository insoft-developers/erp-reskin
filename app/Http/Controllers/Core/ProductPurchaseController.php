...
class ProductPurchaseController extends Controller
{
    ...
    {
        $input = $request->all();
        $query = ProductPurchase::with('product_purchase_item', 'product_purchase_item.product', 'supplier')->where('userid', $this->user_id_staff($input['userid']));
        if (!empty($input['kata_cari'])) {
            $query->whereHas('product_purchase_item.product', function ($q) use ($input) {
    ...
    {
        $input = $request->all();
        $data = Product::where('user_id', $this->user_id_staff($input['userid']))
    ...
    public function store(Request $request)
    {
        ...
        try {
            ...
            for ($i = 0; $i < count($input['product_id']); $i++) {
                ...
                $item->userid = $this->user_id_staff($input['userid']);
                ...
                $this->logStock('md_product', $mp->id, $input['quantity'][$i], 0, $userid, $tanggal_transaksi);
            ...
            if ($input['payment_type'] == 1) {
                ...
                $debt->note = 'Pembelian Barang Dengan Cara Utang';
                $debt->user_id = $this->user_id_staff($input['userid']);
                $debt->sync_status = 0;
                ...
                $_controller = new HutangSyncController();
                $_controller->single_sync($debt->id, $userid);
            }
    ...
    public function destroy(Request $request)
    {
        ...
        if ($dt->sync_status == 1) {
            $journal = Journal::where('relasi_trx', 'purchasing_' . $id)->first();
        ...
        foreach ($items as $item) {
            ...
            $LogStock = LogStock::where('relation_id', $item->product_id)->where('user_id', $this->user_id_staff($userid))->where('table_relation', 'md_product')->where('stock_in', $item->quantity)->whereDate('created_at', $item->created_at)->delete();
        }
    ...
    public function supplier(Request $request)
    {
        ...
        $data = Supplier::where('userid', $this->user_id_staff($input['userid']))->get();
        return response()->json([
            'success' => true,
            'data' => $data,
        ]);
    ...
    public function upload(Request $request)
    {
        ...
        $path = storage_path('app/public/images/product_purchase');
        ...
        try {
            ...
            $data = ProductPurchase::findorFail((int) $ids);
            $data->image = $filename;
            $data->save();
