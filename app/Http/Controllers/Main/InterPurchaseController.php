...
class InterPurchaseController extends Controller
{
    ...
    private $n = 1;
    public function inter_purchase_table()
    {
        $data = InterPurchase::where('userid', $this->user_id_manage(session('id')))->get();
        return DataTables::of($data)
            ...
            ->addColumn('product', function ($data) {
                $materialq = InterProduct::where('id', $data->product_id);
                if ($materialq->count() > 0) {
                    $material = $materialq->first();
                    return $material->product_name . ' - ' . $material->unit;
                } else {
                    return '';
                }
            })
    ...
    {
        $view = 'inter-purchase';
        $materials = InterProduct::where('userid', $this->user_id_manage(session('id')))->get();
        $accounts = DB::table('ml_current_assets')
            ->where('userid', $this->user_id_manage(session('id')))
            ->get();
        return view('main.inter_purchase', compact('view', 'materials', 'accounts'));
    }
    ...
    public function store(Request $request)
    {
        ...
        $this->logStock('md_inter_product', $bsj->id, $input['product_count'], 0, $tanggal_transaksi);
    ...
    public function destroy(string $id)
    {
        ...
        $LogStock = LogStock::where('relation_id', $purchase->product_id)
            ->where('user_id', $this->user_id_manage(session('id')))
            ->where('table_relation', 'md_inter_product')
    ...
    public function change_product_select(Request $request)
    {
        $input = $request->all();

        $pesanan = $input['product_count'];

        $barang_setengah_jadi = InterProduct::find($input['id']);
        $stok_bsj_sekarang = $barang_setengah_jadi->stock;
    ...
    protected function sync_journal($transaction_name, $rf, $st, $nominal, $id, $waktu, $cost_baku, $cost_setengah, $biaya_lain)
    {
        ...
        if ($biaya_lain > 0) {
            $fit = $this->get_account_identity('ml_non_business_expenses', 'biaya-lain-lain', 10);
