...
class ProductPurchaseImport implements ToModel, WithHeadingRow
{
    use CommonTrait;
    ...
    public function model(array $row)
    {
        ...
        if ($this->rowCount == 0) {
            ...
            $gudang = 'Gudang Persediaan Barang Jadi';
            ...
            $purchase_id = DB::table('ml_product_purchases')->insertGetId($data_transaksi);
        ...
        $product = DB::table('md_products')->where('id', $row['product_id'])->first();
        ...
        DB::table('md_products')->where('id', $row['product_id'])->update([
            "quantity" => $stok_awal + $row['jumlah_beli'],
            "cost" => $hpp_masuk
        ]);

        DB::table('log_stocks')->insert([
            "user_id" => $this->user_id_manage(session('id')),
            "relation_id" => $row['product_id'],
            "table_relation" => "md_product",
